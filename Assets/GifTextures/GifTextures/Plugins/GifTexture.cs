...
public class GifTexture : MonoBehaviour
{
        ...
        else
        {
            ...
            foreach (var prefix in prefixes)
            {
                ...
                if (Uri.TryCreate(fullPath, UriKind.Absolute, out gifUri) &&
                    gifUri.IsFile && File.Exists(gifUri.AbsolutePath))
                {
                    glfStream = File.OpenRead(gifUri.AbsolutePath);
#if UNITY_EDITOR
                    m_GifFullPath = gifUri.AbsolutePath;
#endif
                }
            }
    ...
    void ApplyFrame(int frameIndex)
    {
        ...
        int width;
        int height;
        m_GifFrames.GetFrameTexture(frameIndex, out tex, out offsetX, out offsetY, out width, out height);
        ...
        if (componentRenderer != null && componentRenderer.material != null)
        {
            componentRenderer.material.mainTexture = tex;
            componentRenderer.material.mainTextureOffset = new Vector2((float)offsetX / tex.width, (float)offsetY / tex.height);
            componentRenderer.material.mainTextureScale = new Vector2((float)width / tex.width, (float)height / tex.height);
        }
    }
